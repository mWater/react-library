...
// Simple spinner
export function Spinner() { return R('i', {className: "fa fa-spinner fa-spin"}); }
...
let _Button = Button = (function() {
  Button = class Button extends React.Component {
    ...
    render() {
      return R('button', {type: "button", className: classnames("btn", `btn-${this.props.type}`, { active: this.props.active }, { [`btn-${this.props.size}`]: (this.props.size != null) }), onClick: this.props.onClick, disabled: this.props.disabled},
        this.props.children);
    }
  };
  Button.initClass();
...
let _FormGroup = FormGroup = (function() {
  FormGroup = class FormGroup extends React.Component {
    ...
    render() {
      ...
      return R('div', {className: classnames(classes)},
        R('label', {key: "label"}, 
          this.props.labelMuted ?
            R('span', {className: "text-muted"}, this.props.label)
          :
            this.props.label,

          this.props.hint ?
            R('span', {className: "text-muted", style: { fontWeight: this.props.label ? "normal" : undefined }},
              this.props.label ?
                " - " : undefined,
    ...
    render() {
      if (this.props.inline) {
        return R('label', {className: "checkbox-inline"},
          R('input', {type: "checkbox", checked: this.props.value || false, onChange: this.props.onChange ? this.handleChange : undefined}),
          this.props.children);
      } else {
  ...
  Select = class Select extends React.Component {
    ...
    render() {
      ...
      return R('select', {
        ...
        className: classnames("form-control", { "input-sm": this.props.size === "sm" }, { "input-lg": this.props.size === "lg" }),
  ...
  CollapsibleSection = class CollapsibleSection extends React.Component {
    ...
    render() {
      return R('div', {className: "form-group"},
        R('label', {key: "label", onClick: this.handleToggle, style: { cursor: "pointer" }},
          this.state.open ?
...
let _NavPills = NavPills = (function() {
  NavPills = class NavPills extends React.Component {
    ...
    render() {
      return R('ul', {className: "nav nav-pills"}, 
        _.map(this.props.pills, pill => {
          return R('li', {key: pill.id, className: (pill.id === this.props.activePill ? "active" : "")},
            R('a', {href: pill.href, onClick: (() => this.props.onPillClick?.(pill.id))},
              pill.label)
          );
        })
      );
