...
export const GridComponent = (props: {
  ...
  const renderColHeaders = () => {
    ...
    for (let c = visibleRange.colStart; c <= visibleRange.colEnd; c++) {
      ...
      nodes.push(
        <div key={c + ":h"} style={colHeaderStyle}>
          {colHeaderContents}
        </div>
      )
    }
  ...
  const renderRowHeaders = () => {
    ...
    for (let r = visibleRange.rowStart; r <= visibleRange.rowEnd; r++) {
      ...
      nodes.push(
        <div key={"h:" + r} style={rowHeaderStyle}>
          {rowHeaderContents}
        </div>
      )
      y += props.rowHeight
  ...
  const renderEditor = () => {
    ...
    const editorContent = props.renderCellEditor({
      ...
      setSaveEdit: (saveEditFunc: SaveEditFunc) => {
  ...
  const totalWidth =
    props.rowHeaderWidth + props.colWidths.reduce((a, b) => a + b, 0) + (props.colHeaderExtraWidth || 0)
