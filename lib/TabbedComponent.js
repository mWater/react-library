...
class TabbedComponent extends react_1.default.Component {
    constructor(props) {
        ...
        this.renderTab = (tab) => {
            ...
            return R("li", { key: tab.id, className: tabId === tab.id ? "active" : undefined }, R("a", { onClick: this.handleClick.bind(null, tab.id), style: { cursor: "pointer" } }, tab.label, tab.onRemove
                ? R("button", { type: "button", className: "btn btn-xs btn-link", onClick: this.handleRemove.bind(null, tab) }, R("span", { className: "fa fa-times" }))
                : undefined));
        };
    ...
    render() {
        ...
        return R("div", null, R("ul", { key: "tabs", className: "nav nav-tabs", style: { marginBottom: 10 } }, lodash_1.default.map(this.props.tabs, this.renderTab), this.props.onAddTab
            ? R("li", { key: "_add" }, R("a", { onClick: this.props.onAddTab, style: { cursor: "pointer" } }, R("i", { className: "fa fa-plus" })))
