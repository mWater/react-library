...
exports.GridComponent = function (props) {
    ...
    var _a = react_2.useState(null), visibleRange = _a[0], setVisibleRange = _a[1];
    /** Currently selected cell */
    var _b = react_2.useState(null), selection = _b[0], setSelection = _b[1];
    /** Editing state. "none" is not editing, "active" is active editor, "saving" is when save has been requested */
    var _c = react_2.useState("none"), editing = _c[0], setEditing = _c[1];
    ...
    var paneDiv = react_2.useRef(null);
    /** Column resize dragging state */
    var _d = react_2.useState(), colResizing = _d[0], setColResizing = _d[1];
    var _e = react_2.useState(), colResizingDelta = _e[0], setColResizingDelta = _e[1];
    ...
    var updateVisibleRange = react_2.useCallback(function () {
        if (!paneDiv.current) {
            return;
        }
        // Get visible x and y
        var minY = paneDiv.current.scrollTop;
        var maxY = paneDiv.current.scrollTop + paneDiv.current.clientHeight;
        var minX = paneDiv.current.scrollLeft;
        var maxX = paneDiv.current.scrollLeft + paneDiv.current.clientWidth;
        // Determine ranges
        var _a = xyToRowCol(minX, minY), colStart = _a.col, rowStart = _a.row;
        var _b = xyToRowCol(maxX, maxY), colEnd = _b.col, rowEnd = _b.row;
    ...
    var moveSelection = function (sel) { return __awaiter(_this, void 0, void 0, function () {
    ...
    var handleMouseDown = function (ev) {
        ...
        var _a = xyToRowCol(ev.clientX - paneDiv.current.getBoundingClientRect().left + paneDiv.current.scrollLeft, ev.clientY - paneDiv.current.getBoundingClientRect().top + paneDiv.current.scrollTop), row = _a.row, col = _a.col;
    ...
    var handleDoubleClick = function (ev) {
        if (!paneDiv.current) {
            return;
        }
        // Convert to row and col
    ...
    var renderColHeaders = function () {
        ...
        for (var c = visibleRange.colStart; c <= visibleRange.colEnd; c++) {
            ...
            var colHeaderContents = props.renderColHeader({ col: c, width: props.colWidths[c] - 1, height: props.colHeaderHeight - 1 });
    ...
    var renderRowHeaders = function () {
        ...
        for (var r = visibleRange.rowStart; r <= visibleRange.rowEnd; r++) {
            ...
            var rowHeaderContents = props.renderRowHeader({ row: r, width: props.rowHeaderWidth - 1, height: props.rowHeight - 1 });
    ...
    var renderCells = function () {
        ...
        for (var c = visibleRange.colStart; c <= visibleRange.colEnd; c++) {
            ...
            for (var r = visibleRange.rowStart; r <= visibleRange.rowEnd; r++) {
                ...
                nodes.push(react_1.default.createElement("div", { key: c + ":" + r, style: cellStyle }, cellContents));
                y += props.rowHeight;
            }
        }
        return nodes;
    ...
    var totalWidth = props.rowHeaderWidth + props.colWidths.reduce(function (a, b) { return a + b; }, 0);
    var totalHeight = props.rowHeight * props.numRows + props.colHeaderHeight;
    return react_1.default.createElement("div", { key: "pane", style: { width: props.width, height: props.height, overflow: "scroll", position: "relative" }, onScroll: handleScroll, ref: paneRef },
        react_1.default.createElement("div", { key: "inner", onKeyDown: handleKeyDown, tabIndex: 0, onMouseDown: handleMouseDown, onDoubleClick: handleDoubleClick, onMouseUp: handleMouseUp, onMouseMove: handleMouseMove, ref: function (node) { if (node) {
                node.focus();
