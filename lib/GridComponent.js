 ...
 *
 */
exports.GridComponent = (props) => {
    /** Scroll positions. Used to refresh render */
    const [scrollTop, setScrollTop] = react_2.useState(0);
    const [scrollLeft, setScrollLeft] = react_2.useState(0);
    /** Currently selected cell */
    const [selection, setSelection] = react_2.useState(null);
    /** Editing state. "none" is not editing, "active" is active editor, "saving" is when save has been requested */
    const [editing, setEditing] = react_2.useState("none");
    ...
    const editPending = react_2.useRef(false);
    /** Save edit function ref. Editor sets this to block saves */
    const saveEditRef = react_2.useRef(null);
    /** Div of the large inner pane */
    const [paneDiv, setPaneDiv] = react_2.useState(null);
    /** Column resize dragging state */
    const [colResizing, setColResizing] = react_2.useState();
    const [colResizingDelta, setColResizingDelta] = react_2.useState();
    const colResizingStartX = react_2.useRef();
    /** Compute column x coordinates */
    const colXs = react_2.useMemo(() => {
        const xs = [];
    ...
    const paneRef = react_2.useCallback((node) => {
        setPaneDiv(node);
    }, []);
    /** Focus on inner div */
    const innerRef = react_2.useCallback((node) => {
        if (node) {
            node.focus();
        }
    }, []);
    ...
    const renderEditor = () => {
        ...
        const editorContent = props.renderCellEditor({
            ...
            setSaveEdit: (saveEditFunc) => { saveEditRef.current = saveEditFunc; },
