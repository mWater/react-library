...
ReorderableListComponent = (function(superClass) {
  ...
  ReorderableListComponent.prototype.componentWillReceiveProps = function(nextProps) {
    var order;
    if (nextProps.items.length !== this.state.initialOrder.length) {
  ...
  ReorderableListComponent.prototype.fixOrder = function(items) {
    return items.sort((function(_this) {
      return function(left, right) {
        if (_this.state.order.indexOf(_this.props.getItemId(left)) < _this.state.order.indexOf(_this.props.getItemId(right))) {
          return -1;
        }
        if (_this.state.order.indexOf(_this.props.getItemId(left)) > _this.state.order.indexOf(_this.props.getItemId(right))) {
          return 1;
